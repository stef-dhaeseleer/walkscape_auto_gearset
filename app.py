...
from models import Equipment, GearSet
...
def extract_user_counts(user_data: Dict) -> Dict[str, int]:
    ...
    if isinstance(collectibles, list):
        for c in collectibles: counts[str(c).lower()] += 1
    return counts
        ...
        with st.expander("📂 User Save Data & Settings", expanded=True):
            ...
            if user_json_input.strip():
                try:
                    ...
                    st.success(f"Loaded: {user_data.get('name', 'Player')} | Items: {len(item_counts)} | AP: {user_ap}")
    ...
    with left_col:
        ...
        if 'best_gear' in st.session_state:
            ...
            if saved_act_name in act_map:
                ...
                score = optimizer.calculate_score(best_gear, activity, saved_skill_lvl, selected_target, context)
                    ...
                    with tab_exp:
                        ...
                        with d_col2:
                            if swap_item_obj:
                                ...
                                # Analyze New Set
                                analysis = optimizer.analyze_score(test_gear, activity, saved_skill_lvl, selected_target, context)
                                test_score = analysis.get("score", 0)
                                test_steps = analysis.get("denominator", 0)
                                test_formula = analysis.get("formula", "")

                                # Analyze Old Set
                                orig_analysis = optimizer.analyze_score(best_gear, activity, saved_skill_lvl, selected_target, context)
