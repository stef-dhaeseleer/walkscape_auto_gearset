...
def load_collectibles_data() -> List[Collectible]:
    # Assuming a similar path structure for collectibles
    ...
    try:
        ...
        # Fallback or empty if file doesn't exist in environment
...
def extract_user_counts(user_data: Dict) -> Dict[str, int]:
    ...
    # Note: Collectibles are usually boolean (owned or not), but we track count just in case
    # They are stored in a separate list in user_json usually, but if they appear in bank/inv:
    return counts
...
def get_user_collectibles(all_collectibles: List[Collectible], user_data: Dict) -> List[Collectible]:
    ...
    # (Some versions might treat them as items)
    # This part is optional depending on exact save format, but safe to include.
...
def main():
    ...
    with left_col:
        ...
        if run_opt and selected_act_name:
            ...
            with st.spinner(f"Optimizing {selected_act_name}..."):
                best_gear = optimizer.optimize(
                    ...
                    owned_collectibles=owned_collectibles  # Pass collectibles here
                )
        ...
        if 'best_gear' in st.session_state:
            ...
            # Create fresh optimizer for recalculations
            optimizer = GearOptimizer(available_items, all_locations=locations) 

            if saved_act_name in act_map:
                activity = act_map[saved_act_name]

                # We need to calculate the passive stats from collectibles for the display
                ...
                # Combine gear stats + passive stats for display
                ...
                # --- DEBUGGER TABS (Hidden by Default) ---
                st.markdown("---")
                    ...
                    with tab_exp:
                        ...
                        with d_col2:
                            if swap_item_obj:
                                ...
                                # Analyze New Set (passing passive_stats)
                    ...
                    with tab_cand:
                        ...
                        if items:
                            ...
                            for item in items:
                                ...
                                # Use ignore_requirements=True to show stats utility score
                                # Note: passive_stats not included here to show raw item score
